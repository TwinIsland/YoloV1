...
def predict_frame(model, frame):
    """
    Predict output for a single video frame.
    ...
    with torch.no_grad():
        img = Variable(img[None, :, :, :])
        # img = img.cuda()  # Uncomment if using CUDA

        pred = model(img)  # Output shape: 1xSxSx(B*5+C)
        # pred = pred.cpu()  # Uncomment if using CUDA
